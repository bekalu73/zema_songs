...
export const CustomHome = () => {
  const {
    ...
    currentSong,
    songFiltered,
  } = useSelector((store: RootState) => store.songs);
    ...
    <Home>
      ...
      <div style={{ width: "100%", overflow: "hidden" }}>
        ...
        {currentSong.length !== 0 ? (
          <Body>
            {currentBody.toLowerCase() === "artist" &&
              (songFiltered ? (
                <MusicContainer songs={currentSong} />
              ) : (
                <ArtistsList />
              ))}
            {currentBody.toLowerCase() === "genre" &&
              (songFiltered ? (
                <MusicContainer songs={currentSong} />
              ) : (
                <GenreList />
              ))}
            {currentBody.toLowerCase() === "album" &&
              (songFiltered ? (
                <MusicContainer songs={currentSong} />
              ) : (
                <AlbumList />
              ))}
            {currentBody.split(" ")[0].toLowerCase() === "recent" && (
              <MusicContainer songs={recentSongs} />
            )}
            {currentBody.toLowerCase() === "favorite" && (
              <MusicContainer songs={currentSong} />
            )}
            {currentBody.toLowerCase() === "home" && (
              <MusicContainer songs={currentSong} />
            )}
          </Body>
        ) : (
...
const Header = () => {
  return (
    <HeaderContainer>
      <h1 style={{ fontSize: "500%", color: theme.colors.lightWhite }}>
        Empty playlist.
      </h1>
    </HeaderContainer>
  );
